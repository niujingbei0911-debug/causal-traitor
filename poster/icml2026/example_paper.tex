...
\twocolumn[
  \icmltitle{Selective Adversarial Causal Oversight:\texorpdfstring{\\}{}%
    An Information-Partitioned Benchmark Contract and Countermodel-Grounded Reference Verifier}
...
(three seeds, ten samples per family, difficulty $0.55$), the reference
verifier improves the accuracy/unsafe-acceptance/wise-refusal tradeoff over
public-view heuristic baselines, while a deliberately leaking positive control
shows how oracle information can inflate benchmark scores. We additionally run
a five-model live API baseline matrix (Qwen2.5 7B/14B/72B, GPT-5.5 xhigh, and
DeepSeek v4 max-thinking) with raw-response logging and fallback rejection.
These results support the need for public/gold partitioning in causal
oversight benchmarks; larger synthetic runs, real-grounded cases, and human
...
improves verdict accuracy over \textsc{Direct Judge} from $0.504$ to $1.000$ on
\texttt{test\_iid} and from $0.161$ to $0.891$ on \texttt{test\_ood}, while
...
attack-only splits and therefore remains a useful positive control for any
benchmark that does not enforce a public/gold partition; and (c) removing
\emph{abstention} preserves high IID raw accuracy ($0.856$) but collapses
unidentifiable-awareness to $0.00$ on both splits, which is exactly the failure
mode the task is designed to surface. We discuss limitations -- in particular,
the still-modest benchmark scale, missing real-grounded human audit, and API
model-baseline status -- in Section~\ref{sec:experiments} and
...
$0.55$ and compares the full \emph{countermodel-grounded} verifier against
public-view heuristic judge/debate, tool-only and refusal-aware baselines. The
component ablation suite replaces one or more pipeline stages with a no-op surrogate. We
also include a dedicated attack-only leakage study under the
...
verifier, and the current strong-LLM baseline behaviour, not as a final
benchmark release.
...
excluded from the prompt by the same leakage guard used in the smoke tests.
The matrix is small but fully auditable: 30 model/seed/split jobs, $1245$
predictions, raw-response logs, model settings, prompt hashes, and zero
fallback or parse-error records.
...
The strongest raw OOD accuracy among these live LLM baselines is GPT-5.5
...
reference verifier is not evidence that generic LLMs cannot reason causally;
it is evidence that the public/gold partition and three-way selective verdict
surface failure modes that are hidden by closed-world causal QA scoring.
...
($0.877$ vs.\ $0.989$). We therefore interpret the full system as the more
balanced reference verifier, not as uniformly best on every scalar metric.
...
OOD ($1.000$ accuracy), stays strong on attack-family ($0.820$) and
context-shift ($0.883$), and drops sharply on paired-flip ($0.439$). The
paired-flip bucket also has the largest macro-F1 gap ($0.423$), making it the
clearest current stress case for structural disambiguation. The current
significance artifact reports uninformative $p=1$ rows for these bucket gaps,
so we avoid strong significance claims until the OOD estimand and sample count
are upgraded.
...
not yet stable enough to support strong quantitative claims. The API-backed
model evidence now includes a completed five-model baseline matrix with raw
responses and fallback rejection, but it is still only one prompt protocol over
the current synthetic snapshot; larger budgets, more model families, completed
human audit, and pre-registered model/version freezes will be needed before any
broad statement about absolute performance is defensible. The existing transfer
matrix is likewise a
...
Three priorities: (i) closing the real-grounded subset with dual annotation
...
families and sizes; and (iii) extending the completed API baseline matrix to a
pre-registered model/version freeze with more closed and open model families.
