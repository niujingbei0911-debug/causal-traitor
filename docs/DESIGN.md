...
class DifficultyController:
    ...
    def __init__(self):
        self.current_level = 1  # 1-10 连续难度
        self.history = []       # 历史胜负记录
        self.window_size = 5    # 滑动窗口大小

        # Flow通道参数
        self.target_deception_rate = 0.4  # 目标欺骗成功率
        self.tolerance = 0.15             # 容忍区间 [0.25, 0.55]

    def update(self, round_result: dict):
        """每轮结束后更新难度"""
        ...
        deception_rate = sum(
            1 for r in recent if r["a_deceived_successfully"]
        ) / self.window_size

        # 自适应调节
        if deception_rate > self.target_deception_rate + self.tolerance:
            # Agent A 太强 → 增加难度（给C更多工具/线索）
            self._increase_difficulty()
        elif deception_rate < self.target_deception_rate - self.tolerance:
            # Agent A 太弱 → 降低难度（减少C的资源）
    ...
    def _increase_difficulty(self):
        ...
        # 具体措施：增加陪审员数量、给C更多工具、减少A的信息优势

    def _decrease_difficulty(self):
        """对Agent A降低难度"""
        self.current_level = max(1, self.current_level - 1)
        # 具体措施：减少陪审员、限制C的工具、增加A的信息优势

    def get_config(self) -> dict:
        """根据当前难度生成配置"""
        level = self.current_level
        return {
            "n_hidden_vars": 1 + level // 3,
            "noise_std": 0.1 + 0.05 * level,
            "n_confounding_paths": 1 + level // 4,
            "debate_rounds": 3 + level // 2,
            "n_jurors": 3 if level < 7 else 5,
            "agent_a_info": ["full_scm", "partial_scm", "existence_only"][min(level // 4, 2)],
            "agent_c_tools": self._get_tool_set(level)
        }

    def _get_tool_set(self, level):
